  ...
  return (
    <PageWrapper>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard label="Total Chasis"   value={arrivals.length} sub={`${totalPiezas.toLocaleString()} piezas`} />
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {SORT_OPTIONS.map(({ key, label, icon }) => (
          <SortButton key={key} active={orden === key} onClick={() => setOrden(key)} icon={icon}>
            {label}
          </SortButton>
        ))}
      </div>
