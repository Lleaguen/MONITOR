...
export const buildHUData = (
  ...
  CPT_ORDEN.forEach(c => {
    cptData[c] = { zonas: {}, usuariosSetCPT: new Set() };
  });

  const getOrCreateCPT = (cpt) => {
    if (!cptData[cpt]) {
      cptData[cpt] = { zonas: {}, usuariosSetCPT: new Set() };
    }
    return cptData[cpt];
  };
  ...
  csvData.forEach(d => {
    ...
    if (d['Outbound Position'] && outboundId) {
      z.huEnDespachoSet.add(outboundId);
    }

    if (d['Dispatch Included Date'] && dispatchId) {
      const dh = dayjs(d['Dispatch Included Date'], "DD/MM/YYYY HH:mm:ss");
      if (dh.isValid() && dh.hour() >= horaInicioHU) {
    ...
    if (rawUsr) {
      ...
      if (usr) {
        ...
        if (!prev || ts > prev.ts) {
          ultimaActividadUsuario.set(usr, { cpt, zona, ts });
        }
      }
    }
  ...
  Object.keys(cptData).forEach(c => {
    cptData[c].usuariosSetCPT = new Set();
  });
    ...
    .map(cpt => {
      ...
      const zonas = Object.entries(cptData[cpt].zonas).map(([zona, z]) => {
        ...
        const avance = z.etiquetado > 0
          ? Math.round(((z.huCerrado + z.huAbierto) / z.etiquetado) * 100)
          : 0;
  ...
  const horasHasta22 = Math.max(
    dayjs().set('hour', 22).set('minute', 0).set('second', 0)
