...
const SITE_STYLE = {
  CIU: { label: 'CIU · Soldati',     bg: 'bg-blue-500/10',   border: 'border-blue-500/20',   text: 'text-blue-400'   },
  EEV: { label: 'EEV · Echeverría',  bg: 'bg-violet-500/10', border: 'border-violet-500/20', text: 'text-violet-400' },
};

const Header = ({ title, lastUpdate, syncStatus, viewerLastUpdate, selectedSite }) => {
  const site = selectedSite ? (SITE_STYLE[selectedSite] ?? null) : null;
  ...
  return (
    <header className="hidden md:flex justify-between items-center px-6 lg:px-10 py-4 lg:py-6 sticky top-0 bg-[#080c14]/80 backdrop-blur-md z-10 border-b border-white/5">
      <div className="flex items-center gap-4 lg:gap-6">
        ...
        {site && (
          <span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${site.bg} ${site.border} ${site.text}`}>
            {site.label}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-3">
        {/* Pill con hora del último bipeo */}
