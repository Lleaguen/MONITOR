...
/**
 * ArribosPage — página parametrizada de arribs por tipo de vehículo.
 *
 * Props:
 *  data     object  — dashboardData
 *  dataKey  string  — 'arrivalChasis' | 'arrivalCamioneta' | 'arrivalSemi'
 *  label    string  — 'Chasis' | 'Camioneta' | 'Semi'
 */
const ArribosPage = ({ data, dataKey, label }) => {
  const arrivals = data?.[dataKey] || [];
  ...
  const emptyMsg = {
    Chasis:    'Sin datos de chasis',
    Camioneta: 'Sin camionetas pendientes',
    Semi:      'Sin semis pendientes',
  }[label] ?? 'Sin datos';

  return (
    <PageWrapper>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard label={`Total ${label}s`} value={arrivals.length} sub={`${totalPiezas.toLocaleString()} piezas`} />
